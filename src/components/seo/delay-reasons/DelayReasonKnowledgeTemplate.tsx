...
import { localizeDelayReasonAssessmentFi } from "@/lib/localization/delay-reason-assessment-fi";
...
export default function DelayReasonKnowledgeTemplate({ delayReason, locale }: DelayReasonKnowledgeTemplateProps) {
  ...
  const assessment = locale === "sv" ? localizeDelayReasonAssessmentSv(canonicalAssessment) : locale === "da" ? localizeDelayReasonAssessmentDa(canonicalAssessment) : locale === "pl" ? localizeDelayReasonAssessmentPl(canonicalAssessment) : locale === "de" ? localizeDelayReasonAssessmentDe(canonicalAssessment) : locale === "fi" ? localizeDelayReasonAssessmentFi(canonicalAssessment) : canonicalAssessment;
  return <><DelayReasonHero delayReason={delayReason} /><DelayReasonOverview delayReason={delayReason} locale={locale} /><ExtraordinaryCircumstances delayReason={delayReason} locale={locale} /><ClaimAssessment assessment={assessment} locale={locale} /><PassengerRights passengerRights={delayReason.passengerRights} locale={locale} /><CompensationRules compensationRules={delayReason.compensationRules} locale={locale} /><Statistics statisticsIntro={delayReason.statisticsIntro} statistics={delayReason.statistics ?? []} locale={locale} /><Timeline timelineIntro={delayReason.timelineIntro} timeline={delayReason.timeline ?? []} locale={locale} /><FAQ faq={delayReason.faq ?? []} locale={locale} /><RelatedKnowledge delayReasonTitle={delayReason.title} locale={locale} /></>;
}
