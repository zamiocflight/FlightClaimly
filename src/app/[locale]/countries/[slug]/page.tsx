...
import { buildDanishCountryLocalization, danishCountryName } from "@/lib/localization/knowledge-da";
...
function localizeCountry(country: NonNullable<ReturnType<typeof getCountryBySlug>>, locale: string) {
  if (locale === "sv") return applyKnowledgeLocalization(country, buildSwedishCountryLocalization(country));
  if (locale === "da") return applyKnowledgeLocalization(country, buildDanishCountryLocalization(country));
  return country;
}
...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!country) return {};
  return buildMetadata({ entity: localizeCountry(country, locale), locale, pathPrefix: "countries", availableLocales: countrySeoLocales });
}
...
export default async function CountryPage({ params }: PageProps) {
  ...
  const entity = localizeCountry(country, locale);
  const labels = getLocaleDefinition(locale as (typeof countrySeoLocales)[number]).labels;
  const name = locale === "sv" ? swedishCountryName(country.name) : locale === "da" ? danishCountryName(country.name) : country.name;
  ...
  const facts = locale === "sv"
    ...
    : locale === "da"
      ? [{ label: "Land", value: name }, { label: "Regelsæt", value: "EU261 når relevant" }, { label: "Højeste standardkompensation", value: "€600" }]
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <BreadcrumbSchema items={[
        ...
        { name: locale === "sv" ? "Länder" : locale === "da" ? "Lande" : "Countries", url: `https://www.flightclaimly.com/${locale}/countries` },
